...
void roadXML::readNode()
{
    ...
    for (uint i = 0; i < Highways.size(); i++)
    {
        ...
        Highways[i].wayNodeList.assign( Highways[i].nodeIDList.size() , temp );
        uint j = 0;
        for ( auto itr = Highways[i].wayNodeList.begin(); itr != Highways[i].wayNodeList.end(); ++itr, j++ )
        {
            //Highways[i].wayNodeList.insert(Highways[i].wayNodeList.begin()+j , temp1 );
            if (xml0.attributes().value("id").toULong() == Highways[i].nodeIDList[j])
            {              

                Node tempNode;
                //tempNode.sequenceNumber = j;
                itr->nodeID = xml0.attributes().value("id").toULong();
                itr->nodeLatitude = xml0.attributes().value("lat").toDouble();
                itr->nodeLongitude = xml0.attributes().value("lon").toDouble();
                //itr->nodeID = 65;
                std::cout << tempNode.nodeID << " , "  << itr->nodeID << std::endl;
                //Highways[i].wayNodeList.at(j).nodeID = 65;

                std::cout << Highways[i].wayNodeList.at(j).nodeID << std::endl;

                //Highways[i].wayNodeList.emplace( itr , tempNode );

                //Highways[i].wayNodeList.push_back(tempNode);
            }
        }
        std::cout << Highways[i].wayNodeList.at(0).nodeID << std::endl;
        //std::sort(Highways[i].wayNodeList.begin(), Highways[i].wayNodeList.end());
    }
